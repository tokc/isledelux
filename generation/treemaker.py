...
def place_tree(bm):
    ...
    extrude_distance = (random.random() * 0.7) + 0.5 + underground_distance
    ...
    bpy.ops.transform.rotate(
        value=(random.random() * 0.4) - 0.2,
        axis=(random.random(), random.random(), random.random())
    )

    # Save location of top of trunk.
    top_vertices = [v.co for v in ob.data.vertices if v.select]
    top_of_tree = ob.matrix_world * top_vertices[0]
    ...
    # Add material.
    ob.data.materials.append(bpy.data.materials.get('Brown'))
...
def make_tree_top(position=(0.0, 0.0, 0.0), top_size=0.0):
    ...
    material = bpy.data.materials.get('Green')
